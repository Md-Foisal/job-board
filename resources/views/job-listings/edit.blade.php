    ...
    <div class="max-w-4xl mx-auto py-8 px-4">
        ...
        class="flex flex-col gap-4">
            ...
            <label for="expires_at" class="block p-5 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-xl hover:border-zinc-400 dark:hover:border-zinc-500 transition cursor-pointer">
                ...
                <input type="date" name="expires_at" id="expires_at" value="{{ old('expires_at', $jobListing->expires_at?->format('Y-m-d')) }}" class="w-full outline-none px-3 focus:ring focus:ring-zinc-200 dark:focus:ring-zinc-700 py-3 rounded-sm bg-zinc-100 test-zinc-600 dark:bg-zinc-800 dark:text-zinc-300">
                @error('expires_at')
                    <p class="text-sm text-zinc-400 dark:text-zinc-500 mt-2">* {{ $message }}
                    </p>
                @enderror
